...
# Step 5: 显示最终结果
skill_score_labeled.select(
    "Skill", "5", "4", "3", "2", "1", "0", "Proficiency_Levels"
).orderBy("Proficiency_Levels", ascending=False).show(n = 3000,truncate=False)
```
...
```{python}
skills data   Proficiency_Levels
```
...
# ---- 1. 职位要求技能：从 filtered_skills 转换而来 ----
# 你已经有了 PySpark 表 filtered_skills，可以直接转成 Pandas 这里模拟：
required_levels = {
    "SQL": 3,
    "Excel": 3,
    "Python": 3,
...
# ---- 4. 自定义调色热力图（红=需提升，绿=满足）----
plt.figure(figsize=(10, 6))
sns.heatmap(
    df_gap,
    annot=True,
    cmap=sns.diverging_palette(10, 130, s=90, l=50, as_cmap=True),  # 红绿渐变
    center=0,  # 以0为中心（gap=0为白）
    linewidths=0.5,
    cbar_kws={'label': 'Skill Gap (Required - Team)'}
)
